...
function LoadFromServer(fileName) {
    ...
    $.getJSON(fileName, function(JSONdata) {
        BuildContractFromJSON(JSONdata);
    });
}
        ...
        for (var i = dataKeys.length; i > 0; i--) {
            ...
            if (sectionKey != -1) {
                ...
                if (data[dataKeys[i]].hasOwnProperty('numbering')) {
                    ...
                    for (var otherSectionKey = additionalContentToBeRefreshed.length - 1; otherSectionKey >= 0; otherSectionKey--) {
...
function EnableTemplateFileInput() {
    ...
    fileInput.addEventListener('change', function(e) {
        ...
        var isJSON = false;
        if (file.type.match('application/json')) isJSON = true;
        else if (file.name.lastIndexOf(".json") == file.name.length - 5) isJSON = true;
        if (isJSON) {
        ...
        if (!sentAnonymousAgreementUsage) {
            if (!agreementIsCustom) {
                ...
                switch (compensationId) {
                    default:
                        case 0:
                        compensationString = "fixed fee";
                    break;
                    case 1:
                            compensationString = "fixed rate";
                        break;
                    case 2:
                            compensationString = "fee per milestone";
                        break;
                    case 3:
                            compensationString = "percentage of gross receipts";
                        break;
                }
                ...
                switch (rightsId) {
                    default:
                        case 0:
                        rightsString = "company can use contractors work";
                    break;
                    case 1:
                            rightsString = "company will own contractors work";
                        break;
                }
                var version = "";
